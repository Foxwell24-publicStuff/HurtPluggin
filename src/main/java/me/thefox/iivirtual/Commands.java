...
public class Commands
{
    ...
    private boolean checkIIADD(CommandSender sender, Command command, String[] args, Player player, boolean returner)
    {
        if (command.getName().equalsIgnoreCase("IIADD")){
            if (args.length == 0)
            {
                player.sendMessage(ChatColor.RED + "Correct usage: /IIADD <Player> <Type>");
                returner = false;
            }
            if (args.length >= 1)
            {
                ...
                if (iiPlayer != null)
                {
                    if (args.length > 1)
                    {
                        ...
                        {
                            try{ long l = Long.parseLong(args[2]); } catch (NumberFormatException e)
                            { sender.sendMessage(ChatColor.RED + "Not A Number"); return false; }
